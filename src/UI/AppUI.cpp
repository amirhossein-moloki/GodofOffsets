...
void AppUI::Render() {
    ...
    ImGui::Begin("Dashboard & Status", nullptr, ImGuiWindowFlags_NoCollapse);
    RenderHeader();
    ImGui::End();

    auto RenderWindow = [&](const char* title, bool* p_open, TabID index, auto func) {
        if (*p_open) {
            bool attached = m_isAttached;
            if (!attached && index != TabID::Process && index != TabID::ActivityLog) {
                if (ImGui::Begin(title, p_open)) {
                    RenderEmptyState("Attachment required to access this module.", "Go to the Process List and attach to a target first.");
                }
                ImGui::End();
                return;
            }
        ...
        if (m_pm.ReadMemory(m_hexBase, buffer, sizeof(buffer))) {
            for (int i = 0; i < rows; ++i) {
                ...
                for (int j = 0; j < 16; ++j) {
                    ...
                    if (b == 0) ImGui::TextDisabled("%s", byteLabel);
                    else if (b >= 32 && b <= 126) ImGui::TextColored(ImVec4(0.4f, 1.0f, 0.4f, 1.0f), "%s", byteLabel);
                    else if (b == 0xFF) ImGui::TextColored(ImVec4(1.0f, 0.4f, 0.4f, 1.0f), "%s", byteLabel);
                    else ImGui::Text("%s", byteLabel);
                        ...
                        if (ImGui::InputText("##editByte", editBuf, sizeof(editBuf), ImGuiInputTextFlags_CharsHexadecimal | ImGuiInputTextFlags_EnterReturnsTrue)) {
                            try {
                                ...
                                if (m_pm.WriteMemory(currentByteAddr, &newVal, 1)) {
                                    AddLog("Memory written: " + Utils::ToHex(currentByteAddr) + " = " + editBuf, LogSeverity::Success);
                                } else {
