...
std::vector<OffsetResult> OffsetDumper::DumpStructure(uintptr_t baseAddress, const StructDefinition& def, int count) {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        std::string prefix = (count > 1) ? "[" + std::to_string(i) + "] " : "";

        for (const auto& field : def.fields) {
            uintptr_t fieldAddr = currentBase + field.offset;
            std::string value = FormatValue(fieldAddr, field.type);
            results.push_back({ (uintptr_t)(i * def.structSize + field.offset), "", prefix + field.name, field.type, value });
        }
    }
...
std::vector<OffsetResult> OffsetDumper::DumpRange(uintptr_t start, size_t size, const std::string& type) {
    std::vector<OffsetResult> results;
    size_t typeSize = 4;
    if (type == "uintptr_t" || type == "Pointer") {
        typeSize = m_pm.IsTarget64Bit() ? 8 : 4;
    } else if (type.find("64") != std::string::npos || type == "double") {
        typeSize = 8;
    ...
    for (size_t i = 0; i <= (size >= typeSize ? size - typeSize : 0); i += typeSize) {
        std::string val = FormatValue(start + i, type);
...
std::string OffsetDumper::FormatValue(uintptr_t address, const std::string& type) {
    ...
    else if (type == "float" || type == "Float") ss << std::fixed << std::setprecision(4) << m_pm.Read<float>(address);
    else if (type == "double" || type == "Double") ss << std::fixed << std::setprecision(8) << m_pm.Read<double>(address);
