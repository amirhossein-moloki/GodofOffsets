        ...
        if (!m_hProcess.IsValid()) {
            ...
            if (error == ERROR_ACCESS_DENIED) {
                if (EnableDebugPrivilege()) {
                    m_hProcess = Utils::WinHandle(OpenProcess(PROCESS_VM_READ | PROCESS_VM_OPERATION | PROCESS_QUERY_INFORMATION, FALSE, m_pid));
                    if (!m_hProcess.IsValid()) {
                        std::cerr << "[!] Attach failed even with Debug Privilege. Error: " << Utils::GetLastErrorAsString(GetLastError()) << std::endl;
                    }
                } else {
                    std::cerr << "[!] Access Denied and failed to enable Debug Privilege." << std::endl;
                }
            } else {
                std::cerr << "[!] OpenProcess failed for PID " << m_pid << ". Error: " << Utils::GetLastErrorAsString(error) << std::endl;
            }
        }
...
bool ProcessManager::ReadMemory(uintptr_t address, void* buffer, size_t size, bool modifyProtection) const {
    ...
    if (!success) {
        static thread_local uintptr_t lastFailAddr = 0;
        static thread_local DWORD lastError = 0;
        DWORD currentError = GetLastError();
        if (address != lastFailAddr || currentError != lastError) {
