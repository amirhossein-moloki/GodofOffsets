...
void MemoryScanner::ScanRegion(const RegionInfo& region, const ScanValue& val, ScanType scanType, std::vector<uintptr_t>& localResults, std::vector<uint8_t>& localValues, bool modifyProtection) {
    ...
    for (size_t offset = 0; offset < region.size; ) {
        ...
        if (hasAVX2 && toRead >= 32) {
            if (typeSize == 4) {
                uint32_t target;
                if (val.type == DataType::Float) target = std::bit_cast<uint32_t>(std::get<float>(val.value));
                else target = std::get<uint32_t>(val.value);
                ...
                uint64_t target;
                if (val.type == DataType::Double) target = std::bit_cast<uint64_t>(std::get<double>(val.value));
                else target = std::get<uint64_t>(val.value);
